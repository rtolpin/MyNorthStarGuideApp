...
import { initProfile, resetProfile } from './store/slices/userProfileSlice';
...
function GuestApp() {
  ...
  useEffect(() => {
    ...
    dispatch(loadHabits({ habits: [], completions: [] }));
    dispatch(loadGoals([]));
    dispatch(loadBoards([]));
    dispatch(loadCoping({ ratings: [] }));
    dispatch(initProfile('guest'));
  }, [dispatch]);
